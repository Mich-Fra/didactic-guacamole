...
def init_db():
    ...
    cursor.execute("""
        PRAGMA table_info(limiti_ruoli)
    """)
    ...
    if "attivo" not in colonne_limiti:

        cursor.execute("""
            ALTER TABLE limiti_ruoli
    ...
    # --------------------------------------------------------
    # NORMALIZZAZIONE LIMITI ESISTENTI
    # --------------------------------------------------------
...
def applica_protezione_venerdi_lunedi(
    ...
    def ha_lavorato_venerdi_precedente(
        candidato,
        lunedi
    ):

        venerdi = venerdi_precedenti[
            lunedi
        ]
    ...
    for lunedi in lunedi_correnti:
        ...
        offensori = [
            auto_by_id[cid]
            for cid in ids_del_giorno(
                lunedi
            )
            if ha_lavorato_venerdi_precedente(
                auto_by_id[cid],
                lunedi
            )
        ]
        ...
        for offensore in offensori:
            ...
            if (
                int(offensore["id"])
                not in ids_del_giorno(
                    lunedi
                )
            ):
                continue

            if not ha_lavorato_venerdi_precedente(
                offensore,
                lunedi
            ):
                continue
            ...
            for giorno_partner_cand in giorni_candidati:
                ...
                for elemento in list(
                    ...
                    if (
                        limite_partner is not None
                        and conteggi_lun.get(
                            ruolo_partner,
                            0
                        ) > limite_partner
                    ):
                    ...
                    if (
                        limite_off is not None
                        and conteggi_partner.get(
                            ruolo_off,
                            0
                        ) > limite_off
                    ):
...
def pagina_turni():
    ...
    frequenza_rotazione = normalizza_frequenza(
        row[0] if row else 1
    )
    ...
    n_automatici = cursor.fetchone()[0]
    ...
    for (
        ...
        di = parse_date(data_inizio)
        df = parse_date(data_fine)
...
@app.route(
    "/turni/individuale/aggiungi",
    methods=["POST"]
)
def turno_individuale_aggiungi():

    id_dipendente_raw = (
        request.form.get("id_dipendente")
        or ""
    ).strip()

    giorno_forzato = (
        request.form.get("giorno_forzato")
        or ""
    ).strip()

    data_inizio_raw = (
        request.form.get("eccezione_data_inizio")
        or ""
    ).strip()

    data_fine_raw = (
        request.form.get("eccezione_data_fine")
        or ""
    ).strip()
    ...
    if giorno_forzato:
        ...
        if giorno_forzato not in GIORNI_LAVORATIVI:
    ...
    if data_inizio_raw or data_fine_raw:
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM dipendenti
            WHERE id = ?
              AND giorno_forzato IS NOT NULL
              AND giorno_forzato != ''
              AND giorno_forzato != 'Nessuno'
        """, (
            id_dipendente,
        ))

        gia_presente = cursor.fetchone()
        ...
        print(
            "Errore turno_individuale_aggiungi:",
            e
        )
...
# ============================================================
# TURNO INDIVIDUALE - MODIFICA
# ============================================================

@app.route(
    "/turni/individuale/modifica/<int:id_dipendente>",
    methods=["POST"]
)
def turno_individuale_modifica(
    id_dipendente
):

    id_dipendente_nuovo_raw = (
        request.form.get("id_dipendente")
        or ""
    ).strip()
    ...
    try:
        ...
        # ----------------------------------------------------
        # SE CAMBIO DIPENDENTE:
        # IL NUOVO NON DEVE AVERE GIÀ UN TURNO
        # ----------------------------------------------------

        if id_dipendente_nuovo != id_dipendente:
        ...
        print(
            "Errore turno_individuale_modifica:",
            e
        )
...
# ============================================================
# TURNO INDIVIDUALE - RIMUOVI
# ============================================================

@app.route(
    "/turni/individuale/rimuovi/<int:id_dipendente>",
    methods=["POST"]
)
def turno_individuale_rimuovi(
    id_dipendente
):
    ...
    try:
        ...
        print(
            "Errore turno_individuale_rimuovi:",
            e
        )
...
def periodo_elimina(id_periodo):
